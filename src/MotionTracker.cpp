...
void MotionTracker::startRecording() {
  clearRecording();
  is_recording = true;
  recording_start_time = millis();
}

void MotionTracker::stopRecording() {
  is_recording = false;
}

void MotionTracker::clearRecording() {
  step_count = 0;
  recording_start_time = 0;
}

bool MotionTracker::isRecording() const {
  return is_recording;
}

void MotionTracker::captureStep() {
  if (!is_recording || step_count >= MAX_STEPS) {
    return;
  }
  
  MovementStep& step = steps[step_count];
  step.timestamp = millis() - recording_start_time;
  ...
  if(grabber != nullptr) {
    step.grabber_active = grabber->isActive();
  }
  
  if(magnet != nullptr) {
    step.magnet_active = magnet->isActive();
  }
  
  step_count++;
}

void MotionTracker::playSequence(bool loop) {
  do {
    for(int i = 0; i < step_count; i++) {
      ...
      if(i < step_count - 1) {
        unsigned long delay_time = steps[i + 1].timestamp - step.timestamp;
        delay(delay_time);
      }
    }
  } while(loop);
...
void MotionTracker::playSequenceReverse() {
  for(int i = step_count - 1; i >= 0; i--) {
    ...
    if(i > 0) {
      unsigned long delay_time = step.timestamp - steps[i - 1].timestamp;
      delay(delay_time);
    }
  }
}

void MotionTracker::returnToOrigin() {
  if(step_count > 0) {
...
void MotionTracker::printRecording() const {
  ...
  for(int i = 0; i < step_count; i++) {
    ...
    Serial.print(i);
    Serial.print(" @ ");
    Serial.print(steps[i].timestamp);
    Serial.print("ms: [");
    for(int j = 0; j < 4; j++) {
